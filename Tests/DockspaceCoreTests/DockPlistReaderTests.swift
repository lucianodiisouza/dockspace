...
final class DockPlistReaderTests: XCTestCase {
  func test_read_decodesAppsSpacersAndOthersFromFixture() throws {
    ...
    XCTAssertEqual(
      snapshot.apps[0], .app(AppEntry(path: "/Applications/Safari.app", displayName: "Safari")))
    XCTAssertEqual(
      snapshot.apps[1], .app(AppEntry(path: "/Applications/Mail.app", displayName: "Mail")))
    ...
    XCTAssertEqual(
      snapshot.others[0],
      .file(FileEntry(path: "/Users/Shared/Documents", displayName: "Documents", isDirectory: true))
    )
    XCTAssertEqual(
      snapshot.others[1], .url(URLEntry(url: "https://example.com", displayName: "Example")))
  }
